...
class Measure(depth_frame, color_frame):

    def __init__(self, depth_frame, color_frame,depth_scale):
    ...
    min_dist = 
    def blocked(self, point, obstacles, min_dist=0.2/self.depth_scale):
        """
        ...
        is_blocked = False
        i = 0
        while is_blocked == False and i < len(obstacles):
            if self.measure(point,obstacles[i]) < min_dist:
                is_blocked = True
            else:
                i += 1
